...
/**
 * 文件上传控制器
 * 提供文件分片上传、状态查询、文件合并等功能。
 * 支持大文件的分片上传，并通过 Kafka 进行异步文件处理。
 * 主要功能：
 *     分片上传：支持大文件分片上传，支持断点续传
 *     状态查询：查询文件上传进度
 *     文件合并：将所有分片合并为完整文件
 *     文件类型验证：验证上传文件的类型是否支持
 *
 * @author PaiSmart Team
 * @since 1.0
 */
@RestController
...
public class UploadController {
    ...
    /** Kafka 消息模板，用于发送文件处理任务 */
    @Autowired
    private KafkaTemplate<String, Object> kafkaTemplate;

    /** Kafka 配置，提供主题名称等配置信息 */
    @Autowired
     ...
     * 构建文件已合并的响应
     * <p>
     * 用于幂等处理：当文件已经完成合并时，直接返回已存在的文件URL，
     * 避免重复处理。
     * </p>
     *
     * @param fileMd5 文件的MD5值
     * @return 包含文件访问URL的成功响应
     * @throws Exception 生成文件URL时可能抛出的异常
     */
