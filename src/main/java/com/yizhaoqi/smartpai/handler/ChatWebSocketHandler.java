...
/**
 * WebSocket聊天处理器
 * 处理客户端WebSocket连接的建立、消息收发和连接关闭等生命周期事件
 * 支持JWT令牌认证、心跳检测和停止响应等特性
 */
@Component
public class ChatWebSocketHandler extends TextWebSocketHandler {
    ...
    /**
     * WebSocket连接建立时的处理
     * 验证JWT令牌，建立用户与会话的映射关系，并向客户端发送连接确认消息
     */
    @Override
    ...
    private String extractUserId(String jwtToken) {
        ...
        if (userId == null || userId.isBlank()) {
            throw new IllegalArgumentException("无法从 JWT令牌中提取用户ID");
        }
